...
if(!testing){
    pdf(
        file=here('figures/posterior_monoculture_area.pdf'),
        width=14,
        height=10
    )
}else{
...
for(tmodel in c("Beverton-Holt","Ricker")){
    for(vmodel in c("Beverton-Holt","Ricker")){
        ...
        if(vmodel=="Beverton-Holt"){
            ...
            if(tmodel=="Ricker"){
                ...
                text(
                    x=0,
                    y=14,
                    label=expression('Size of the area in monoculture, log('*italic(A[gamma])*')'),
                    # side=2,
                    cex=2.5,
                    # line=3.5,
                    srt=90,
        ...
        if(tmodel=="Beverton-Holt"){
            if(vmodel=="Beverton-Holt"){
                text(1.5,16,vmodel,adj=c(0.5,0.5),cex=2.2,xpd=NA)
            }else{
                text(1.5,11.6,vmodel,adj=c(0.5,0.5),cex=2.2,xpd=NA)
            }
