...
vero_0<-ggplot(vero) +
  geom_density(mapping = aes(x = lambda, fill = model, linetype=model) ,
...
vero_1<-ggplot(vero) +
  geom_density(mapping = aes(x = lambda_env, fill = model, linetype=model) ,
               alpha = 0.8,show.legend = FALSE) +
  scale_linetype_manual(values=c("solid","dashed", "dotted"))+
  scale_fill_manual(values=palette_alba) +
  theme_alba+
...
pdf(file = "../bayesian_competition_ms/lambda.pdf", width = 8, height = 6)
annotate_figure(all_figures,
                
                bottom = text_grob("Fecundity in the absence of competition",
                                   size = 12),
