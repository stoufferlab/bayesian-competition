...
pdf(
    file=here("figures/posterior_feasibility_domains_medians.pdf"),
    width=7,
    height=7
)
...
for(tmodel in c('beverton_holt','ricker')){
    for(vmodel in c('beverton_holt','ricker')){
        ...
        model_combo <- paste('v',vmodel, 't',tmodel, sep="_")
        # xmin[[vmodel]] <- Inf
        # ymin[[tmodel]] <- Inf
        # xmax[[vmodel]] <- 0
        # ymax[[tmodel]] <- 0
        ...
        # plot domains for both environments
        for(woody in c(1,0)){
                ...
                for(i in rev(seq.int(nrow(posteriors[["vero"]][[vmodel]])))){
                    ...
                    if(woody==1){
                        ...
                        points(
                            ...
                            bg='white' #scales::alpha("white", alpha = 0.1)
                        )
                    }
                }
            }
                ...
                if(cntr %in% c(2,3)){
                    mtext(
                        bquote(.(vtext) ~ "vital rate," ~ italic(r[i])),
                        side = 1,
                        # outer = TRUE,
                        line=2.75,
                        cex=1.2
                ...
                if(cntr %in% c(0,2)){
                    mtext(
                        bquote(.(ttext) ~ "vital rate," ~ italic(r[j])),
                        side = 2,
                        line=2.25,
                        # outer = TRUE,
                        cex=1.2
